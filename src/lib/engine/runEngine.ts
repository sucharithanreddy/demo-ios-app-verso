...
function buildAnalysisPrompt(userProfile?: UserProfile): string {
  const profileBlock = userProfile ? buildUserProfileBlock(userProfile) : '';
...
function buildUserProfileBlock(profile: UserProfile): string {
  const p = (profile.primaryProfile || '').toLowerCase();
  const s = (profile.secondaryProfile || '').toLowerCase();
  ...
  const secondaryGuide = s && archetypeGuidance[s]
    ? ` Their secondary archetype is ${profile.secondaryProfile}, which can amplify or moderate these tendencies.`
    : '';

  const scoreLine = (profile.driverScore !== undefined && profile.strategistScore !== undefined
    && profile.connectorScore !== undefined && profile.reactorScore !== undefined)
    ? ` Pattern strengths (0-100): Driver ${profile.driverScore}, Strategist ${profile.strategistScore}, Connector ${profile.connectorScore}, Reactor ${profile.reactorScore}.`
    : '';
...
export interface RunEngineInput {
  ...
  /**
   * Optional Sales Wellbeing Map profile, used to personalize the AI's
   * response based on the user's dominant pressure archetype.
   *
   * When provided, the analysis + response prompts include a short
   * "USER PROFILE" block telling the LLM how this user typically
   * experiences sales pressure (e.g. Driver → tends to push harder under
   * stress; Connector → takes rejection personally). This delivers the
   * Phase 3 "AI Companion" promise from the product briefing.
   *
   * Backwards-compatible: callers that don't pass this get identical
   * behaviour to before.
   */
  userProfile?: UserProfile;
}
...
/**
 * Subset of DiagnosticResult that the engine cares about.
 * Kept as a structural type so we can accept either a Prisma row or a
 * plain object built from localStorage.
 */
...
export async function runEngine(input: RunEngineInput) {
  const { userText, conversationHistory = [], sessionContext, userProfile } = input;
  ...
  // Phase 1: Analysis
  const analysisMessages: AIMessage[] = [{ role: 'system', content: buildAnalysisPrompt(userProfile) }];
