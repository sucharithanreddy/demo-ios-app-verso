...
import { SafeSignInButton, SafeUserButton } from '@/lib/safe-auth';
...
export default function LabPage() {
  const { hasAccess: isSignedIn, isLoaded } = useDemoAccess();
  ...
  // Redirect to diagnostic results if no access - user needs to unlock via test
  if (!isSignedIn) {
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden noise">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-8 md:space-y-10 pb-mobile">
        {/* Sales Wellbeing Diagnostic - Featured */}
        <section>
          <motion.div
          ...
          >
            <Link href="/diagnostic" className="block">
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/90 via-primary to-accent p-6 shadow-premium hover:shadow-lg transition-all cursor-pointer group">
                {/* Decorative elements */}
